        ...
        <div class="d-flex justify-content-start mb-3">
            <form action="{{ Route('candidate') }}" class="d-flex">
              <select class="form-control" name="position" id="position">
                ...
                <a href="/jobs/candidates" class="btn btn-outline-primary text-nowrap ml-1">
                   ...
                   @foreach ($registrations as $i => $registration)
                    <tr>
                        <td>{{(($registrations->currentPage() - 1 ) * $registrations->perPage() ) + $i + 1}}</td>
                            <td> <a href="{{Route('candidate.edit',$registration)}}">{{$registration->name}} </a> </td>
                            <td> <a href="{{Route('candidate.edit',$registration)}}"> {{$registration->email}} </a></td>
                            ...
                            <td class="text-center">  
                                ...
                                @case('รอตรวจสอบ')
                                    ...
                                    @break
                                    @case('รอสัมภาษณ์งาน')
                                   <h5> <span class="badge badge-info  ">รอสัมภาษณ์งาน</span></h5>
                                    @break
                                    @case('ผ่านการสัมภาษณ์งาน')
                                    <h5><span class="badge badge-success ">ผ่านการสัมภาษณ์งาน</span></h5>
                                    @break
                                    @case('ไม่ผ่านการสัมภาษณ์งาน')
                                    <h5><span class="badge badge-danger">ไม่ผ่านการสัมภาษณ์งาน</span></h5>
                                    @break
                                    @case('รอผลการสรุป')
                                    <h5><span class="badge badge-light">รอผลการสรุป</span></h5>
                                    @break
                            ...
                            @endswitch  
                                ...
                                <td> <a href="{{Route('candidate.edit',$registration)}}" class="btn btn-outline-primary">รายละเอียด</a></td>
